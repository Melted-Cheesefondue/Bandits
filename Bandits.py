...
def show_human(im,pl): #shows an image of the human
    global human_position
    im.remove()
...
def control(event): #changes the colour of the background if an impossible manoever is tried when manually moving the robot
    global im, go, fig
    ...
    if key_pressed is not None and key_pressed != 'shift':
        ...
        if val==0:
            ...
            im = show_human(im,plt,human_position)
    if key_pressed.upper()=='Q':
        go = False



# the next function concern the AI
